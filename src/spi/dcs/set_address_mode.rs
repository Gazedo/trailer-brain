...
use crate::spi::options::{ColorOrder, HorizontalRefreshOrder, Orientation, RefreshOrder, VerticalRefreshOrder, ModelOptions};
...
impl DcsCommand for SetAddressMode {
    ...
    fn fill_params_buf(&self, buffer: &mut [u8]) -> Result<usize, DisplayError> {
        buffer[0] = self.0;
        Ok(1)
    }
}
